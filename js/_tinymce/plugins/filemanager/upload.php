...
if (!empty($_FILES) && $upload_files) {
    ...
    $targetPathThumb = dirname( __FILE__ ) . '/'. $storeFolderThumb . '/'; 
     
	// เปลี่ยนชื่อไฟล์ที่อัพโหลด
	$ext = pathinfo($_FILES['file']['name'], PATHINFO_EXTENSION);
	//	$new_flies_name = date('YmdHis').'.'. $ext;
	$new_flies_name = basename($_FILES["file"]["name"],".".$ext).".".$ext;	// เปลี่ยนชื่อไฟล์ที่อัพโหลด
	
    $targetFile =  $targetPath. $new_flies_name;
    $targetFileThumb =  $targetPathThumb. $new_flies_name; 
	...
	//	if($is_img){
    if($is_img && $ext!="gif") {		//	.gif ไม่สามารถ resize ได้
